...
public class WorldGenFireDragonRoosts extends WorldGenerator implements WorldGenStructTrait{
            ...
            for (BlockPos blockpos : BlockPos.getAllInBox(position.add(-j, -k, -l), position.add(j, k, l))) {
                if (blockpos.distanceSq(position) <= (double) (f * f)) {
                    ...
                    if (null!=height_df&&dist < 0.3D && rand.nextInt(isMale ? 250 : 400) == 0) {
                        new WorldGenRoostGoldPile(IafBlockRegistry.goldPile).generate(worldIn, rand, height_df);
                    }
