...
public class CBiomeTemperature implements WGCondition{
	...
	public void fromJson(JsonObject args) {
		if(args.has("min")&&args.get("min").isJsonObject()) {
			this.setMin(true,
					...
					args.get("min").getAsJsonObject().get("value").getAsFloat()
			);
		}else {
		...
		if(args.has("max")&&args.get("max").isJsonObject()) {
			this.setMax(true,
					...
					args.get("max").getAsJsonObject().get("value").getAsFloat()
			);
		}else {
	...
	public void setMin(boolean exist,boolean canEq,float val) {
		if(exist) {
			this.minExist=true;
			this.minValue=val;
			this.minEqual=canEq;
	...
	public void setMax(boolean exist,boolean canEq,float val) {
		if(exist) {
			this.maxValue=val;
			this.maxEqual=canEq;
			this.maxExist=true;
	...
	protected boolean checkDefault(float v1) {
        if(this.minExist) {
        	if(v1<this.minValue) {
        		return false;
        	}
	...
	public boolean apply(WGParam args) {
		float v1 = args.biome.getDefaultTemperature();
        return this.checkDefault(v1);
	}
