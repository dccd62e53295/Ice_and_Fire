...
import com.github.alexthe666.iceandfire.event.worldgen.WGDim;
...
*/
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator, IChunkProvider chunkProvider) {
    	...
    	WGDim config=WGEventRoot.fetchConfig(world);
        ...
        WGParam param=new WGParam(random, chunkX, chunkZ, world, chunkGenerator, chunkProvider);
        
        config.generate(param);
        /*
        double spawnCheck = param.spawnCheck;
        int x = param.x;
        int z = param.z;
        BlockPos height = param.surfacePos;
         
          // base: 428400
         
        
        // 75 => 5712
        if (IceAndFire.CONFIG.spawnGorgons && !isDimensionBlacklisted(world.provider.getDimension(), false) && 
        		(lastGorgonTemple == null || lastGorgonTemple.distanceSq(height) >= spawnCheck)
        		) {
            if (BiomeDictionary.hasType(world.getBiome(height), Type.BEACH)) {
                if (random.nextInt(IceAndFire.CONFIG.spawnGorgonsChance + 1) == 0) {
                    BlockPos surface = height;
        ...
        // 300 => 1428
        if (IceAndFire.CONFIG.generateSirenIslands && isFarEnoughFromSpawn(world, height) && 
        		BiomeDictionary.hasType(world.getBiome(height), Type.OCEAN) && 
        		!BiomeDictionary.hasType(world.getBiome(height), Type.COLD)
                && random.nextInt(IceAndFire.CONFIG.generateSirenChance + 1) == 0 && 
                !isDimensionBlacklisted(world.provider.getDimension(), false) && (lastSirenIsland == null || lastSirenIsland.distanceSq(height) >= spawnCheck)) {
            SIREN_ISLAND.generate(world, random, height);
            lastSirenIsland = height;
        }

        // 170 => 2520
        if (IceAndFire.CONFIG.generateCyclopsCaves && isFarEnoughFromSpawn(world, height) && 
        		BiomeDictionary.hasType(world.getBiome(height), Type.BEACH)
                && random.nextInt(IceAndFire.CONFIG.spawnCyclopsCaveChance + 1) == 0 && 
                world.getBlockState(height.down()).isOpaqueCube() && 
                !isDimensionBlacklisted(world.provider.getDimension(), false) && (lastCyclopsCave == null || lastCyclopsCave.distanceSq(height) >= spawnCheck)) {
            CYCLOPS_CAVE.generate(world, random, height);
            lastCyclopsCave = height;
        }
        
        // 900 => 476
        if (IceAndFire.CONFIG.generateWanderingCyclops && isFarEnoughFromSpawn(world, height) && 
        		BiomeDictionary.hasType(world.getBiome(height), Type.PLAINS) && 
        		(lastCyclopsCave == null || lastCyclopsCave.distanceSq(height) >= spawnCheck)) {
        ...
        if (IceAndFire.CONFIG.spawnPixies && 
        		...
        		(lastPixieVillage == null || lastPixieVillage.distanceSq(height) >= spawnCheck)
        		) {
        ...
        if (IceAndFire.CONFIG.generateDragonRoosts && 
            ...
            if (!world.getBiome(height).getEnableSnow() && 
            		world.getBiome(height).getDefaultTemperature() > 0.0 && 
            		world.getBiome(height) != Biomes.ICE_PLAINS && 
            		!BiomeDictionary.hasType(world.getBiome(height), Type.COLD) && 
            		!BiomeDictionary.hasType(world.getBiome(height), Type.SNOWY) && 
            		!BiomeDictionary.hasType(world.getBiome(height), Type.WET) && 
            		!BiomeDictionary.hasType(world.getBiome(height), Type.OCEAN) && 
            		!BiomeDictionary.hasType(world.getBiome(height), Type.RIVER)
            		) {
            ...
            if (BiomeDictionary.hasType(world.getBiome(height), Type.COLD) && 
            		BiomeDictionary.hasType(world.getBiome(height), Type.SNOWY)
            		) {
                if (random.nextInt((isHills ? IceAndFire.CONFIG.generateDragonRoostChance : IceAndFire.CONFIG.generateDragonRoostChance * 2) + 1) == 0) {
                    BlockPos surface = world.getHeight(new BlockPos(x, 0, z));
        ...
        if (IceAndFire.CONFIG.generateDragonSkeletons && 
            ...
            if (BiomeDictionary.hasType(world.getBiome(height), Type.DRY) && 
            		BiomeDictionary.hasType(world.getBiome(height), Type.SANDY) && 
            		random.nextInt(IceAndFire.CONFIG.generateDragonSkeletonChance + 1) == 0
            		) {
        ...
        // 70 => 6120
        if (IceAndFire.CONFIG.spawnHippocampus && 
        		BiomeDictionary.hasType(world.getBiome(height), Type.OCEAN) && 
        		random.nextInt(IceAndFire.CONFIG.hippocampusSpawnChance + 1) == 0
        		) {
        ...
        // 200 => 2142
        if (IceAndFire.CONFIG.spawnSeaSerpents && 
        		BiomeDictionary.hasType(world.getBiome(height), Type.OCEAN) && 
        		random.nextInt(IceAndFire.CONFIG.seaSerpentSpawnChance + 1) == 0
        		) {
            BlockPos pos = new BlockPos(x + random.nextInt(10) - 5, 20 + random.nextInt(40), z + random.nextInt(10) - 5);
            if (world.getBlockState(pos).getMaterial() == Material.WATER) {
        ...
        // 100 => 4284
        if (IceAndFire.CONFIG.spawnStymphalianBirds && 
        		isFarEnoughFromSpawn(world, height) && 
        		BiomeDictionary.hasType(world.getBiome(height), Type.SWAMP) && 
        		random.nextInt(IceAndFire.CONFIG.stymphalianBirdSpawnChance + 1) == 0
        		) {
        ...
        // 180 => 2380, isHills ? 180 : 360
        if (IceAndFire.CONFIG.generateDragonDens && 
        		isFarEnoughFromSpawn(world, height) && 
        		!isDimensionBlacklisted(world.provider.getDimension(), true) && 
        		(lastDragonCave == null || lastDragonCave.distanceSq(height) >= spawnCheck)
        		) {
        ...
        if (IceAndFire.CONFIG.generateSnowVillages && 
        		...
        		BiomeDictionary.hasType(world.getBiome(height), Type.COLD) && 
        		BiomeDictionary.hasType(world.getBiome(height), Type.SNOWY) && 
        		(lastSnowVillage == null || lastSnowVillage.distanceSq(height) >= spawnCheck)
        		) {
...
/*
    private boolean isDimensionBlacklisted(int id, boolean dragons) {
     ...
     int[] dragonBlacklistedDimensions = new int[] { 1, -1 };
     int[] dragonWhitelistedDimensions = new int[] { 0 };
     boolean useDimensionBlackList = true;
     int[] structureBlacklistedDimensions = new int[] { 1, -1 };
     int[] structureWhitelistedDimensions = new int[] { 0 };

        boolean useBlackOrWhiteLists = useDimensionBlackList;
        int[] blacklistedArray = dragons ? dragonBlacklistedDimensions : structureBlacklistedDimensions;
        int[] whitelistedArray = dragons ? dragonWhitelistedDimensions : structureWhitelistedDimensions;
